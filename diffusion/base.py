...
from utils import to_cpu, MutualInformation 
...
class GaussianDiffusionBeatGans:
    ...
    def training_losses(self,
        ...
        Proto=False,
        alpha=0, beta=0,
        network =None,  epoch=0, updater=None, 
        noise: th.Tensor = None, model_kwargs=None, load_in=0):
        """
        ...
        if self.loss_type in [
            ...
            net_loss = self.calc_net_loss(network, embeddings, labels, loss, updater=updater) * beta if epoch > load_in else 0  


            # contrastive_loss = self.calc_contrastive_loss(embeddings, labels) 
            contrastive_loss = 0 
            ...
            if self.loss_type == LossType.mse:
                if self.model_mean_type == ModelMeanType.eps:
                  if Proto:
                    ...
                    terms['net-loss'] = net_loss 
                    terms['contrastive-loss'] = contrastive_loss 
                    terms["proto-diffae"] = contrastive_loss + net_loss  
    ...
    def calc_net_loss(self, network, embeddings, y, loss, updater=None):
      y_hat = network.forward(embeddings)
                       
      l = loss(y_hat.reshape(-1, 1), y.to(torch.float32).reshape(-1, 1))      


        #calculate the l1 norm of the network weights
