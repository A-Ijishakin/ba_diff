...
class LitModel(pl.LightningModule):
    ...
    def forward(self, x=None, noise=None, x_start=None, ema_model: bool = False):
        with amp.autocast(False):
            ...
            if x is not None:     
                ...
                gen = self.sampler.sample(model=model,
                                        noise=noise,
                                        x_start=x_start)
            return gen
    ...
    def training_epoch_end(self, outputs, gen= False) -> None:
        if self.current_epoch > self.conf.load_in: 
            batch_size = ... 
            train_loader = torch.utils.data.DataLoader(ExampleDataset(train=True), shuffle = False, batch_size=batch_size) 
            #3374
            val_loader = torch.utils.data.DataLoader(ExampleDataset(val = True), shuffle = False, batch_size=batch_size) 
            ...
            with torch.no_grad():
                for loader in loaders: 
                    Y, Y_PRED = [], [] 
                    mode = 'Training' if loader == train_loader else 'Validation'
                    for data in loader: 
                        x = data['img'].to('cuda')
                        y = data['label'] 
                        z_sem = encoder(x) 
                    ...
                    Y, Y_PRED = [int(y) for sub_array in Y for y in sub_array], [int(y) for sub_array in Y_PRED for y in sub_array]
                    ...
                    self.logger.experiment.add_scalar(f'{mode} MAE', mae, 
                    self.current_epoch) 

                    self.logger.experiment.add_scalar(f'{mode} R', r, 
                                                        self.current_epoch) 
                    
                    self.logger.experiment.add_scalar(f'{mode} R2', r2, 
                                        self.current_epoch) 
        ...
        with amp.autocast(False):
            ...
            if self.conf.train_mode == TrainMode.diffusion:
                ...
                losses = self.sampler.training_losses(model=self.model,
                                    x_start=x_start,
                                    t=t, labels=labels, Proto=self.conf.Proto, 
                                    alpha=self.conf.alpha,  beta = self.conf.beta,  
                                    network = self.model.network,
                                   epoch=self.current_epoch, load_in=self.conf.load_in) 
            ...
            normie = 0.0000 * torch.norm(torch.cat([param.data.view(-1) for (name, param) in self.model.named_parameters() if 'encoder' in name]), p=1) 

            loss = losses['mse'].mean() if not self.conf.Proto else losses['mse'].mean() + losses['proto-diffae'] + normie 
            ...
            if self.global_rank == 0:
                ...
                self.logger.experiment.add_scalar('network-loss', losses['net-loss'],
                                                    self.num_samples)
                ...
                self.logger.experiment.add_scalar('lr-param-group1', self.optimizer.param_groups[0]['lr'],
                                                    self.num_samples)   
                self.logger.experiment.add_scalar('lr-param-group2', self.optimizer.param_groups[1]['lr'],
                                                    self.num_samples)
    ...
    def configure_optimizers(self, oldie=False):
        out = {}
        if self.conf.optimizer == OptimizerType.adam: 
            all_parameters = [param for (name, param) in self.model.named_parameters() if 'network' not in name]  
            optim = torch.optim.Adam(params=[{'params':all_parameters}, {'params':self.model.network.parameters()}]) 
            optim.param_groups[0]['lr'] = self.conf.lr 
